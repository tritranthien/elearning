...
import type { Route } from "./+types/dictionary-quiz";
import { prisma } from "../utils/db.server";
import { requireUserId } from "../utils/session.server";

export function meta({ }: Route.MetaArgs) {
    return [{ title: "Kiểm tra từ vựng - LinguaFast" }];
}

export async function loader({ request }: Route.LoaderArgs) {
    await requireUserId(request);
    const url = new URL(request.url);
    const filter = url.searchParams.get("filter") || "all"; // all, uncategorized, topic
    ...
    const words = await prisma.word.findMany({
        ...
        include: {
            topic: {
                select: { title: true, viTitle: true, slug: true }
            }
        },
        orderBy: { term: "asc" }
    });
    ...
    const topics = await prisma.topic.findMany({
        select: { id: true, title: true, viTitle: true, slug: true },
        orderBy: { title: "asc" }
    });

    return { words, topics, filter };
}
...
export default function DictionaryQuiz() {
    const { words, topics, filter } = useLoaderData<typeof loader>();
    ...
    const getFilterLabel = () => {
        if (filter === "uncategorized") return "Chưa phân loại";
        return "Tất cả từ điển";
    };
        ...
        return (
            <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 flex items-center justify-center p-4">
                <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full text-center">
                    ...
                    <h1 className="text-2xl font-black text-gray-900 mb-2">Chưa đủ từ vựng</h1>
                    <p className="text-gray-500 mb-6">
                        Cần ít nhất 4 từ trong danh mục "{getFilterLabel()}" để tạo bài kiểm tra
                    </p>

                    <div className="space-y-3 mb-6">
                        <button
                            onClick={() => handleFilterChange("all")}
                            className={`w-full py-3 rounded-xl font-bold transition-all ${filter === "all" ? "bg-primary text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
                        >
                            📚 Tất cả từ điển ({words.length} từ)
                        </button>
                        <button
                            onClick={() => handleFilterChange("uncategorized")}
                            className={`w-full py-3 rounded-xl font-bold transition-all ${filter === "uncategorized" ? "bg-amber-500 text-white" : "bg-amber-50 text-amber-600 hover:bg-amber-100"}`}
                        >
                            📂 Chưa phân loại
                        </button>
                    </div>
                ...
                <div className="container max-w-2xl mx-auto py-12">
                    <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
                        <div className={`p-8 text-center ${percentage >= 80 ? 'bg-gradient-to-r from-emerald-500 to-teal-500' : percentage >= 50 ? 'bg-gradient-to-r from-amber-500 to-orange-500' : 'bg-gradient-to-r from-rose-500 to-pink-500'}`}>
                            ...
                            <p className="text-white/60 text-sm mt-1">{getFilterLabel()}</p>
                        </div>
            ...
            <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 flex items-center justify-center p-4">
                <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-gray-500 font-medium">Từ điển LinguaFast</p>
                    </div>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="space-y-2">
                            <button
                                onClick={() => handleFilterChange("all")}
                                className={`w-full py-3 px-4 rounded-xl font-bold transition-all flex items-center justify-between ${filter === "all" ? "bg-primary text-white shadow-lg" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
                            >
                                <span>📚 Tất cả từ điển</span>
                                <span className="text-sm opacity-70">{words.length} từ</span>
                            </button>
                            <button
                                onClick={() => handleFilterChange("uncategorized")}
                                className={`w-full py-3 px-4 rounded-xl font-bold transition-all flex items-center justify-between ${filter === "uncategorized" ? "bg-amber-500 text-white shadow-lg" : "bg-amber-50 text-amber-600 hover:bg-amber-100"}`}
                            >
                                <span>📂 Chưa phân loại</span>
                                <span className="text-sm opacity-70">{filter === "uncategorized" ? words.length : "—"} từ</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        <button
                            ...
                            className="w-full py-4 bg-gradient-to-r from-primary to-indigo-600 text-white font-black rounded-xl hover:shadow-lg transition-all text-lg disabled:opacity-50"
                        >
                            Bắt đầu kiểm tra
                        </button>
                        <Link
